...
void _testEncodeDecodeRandom(GenericGF field, int dataSize, int ecSize) {
  ...
  for (int i = 0; i < iterations; i++) {
    ...
    _testDecoder(field, dataWords, ecWords);
  }
}

void _testEncodeDecode(
    GenericGF field, List<int> dataWords, List<int> ecWords) {
  _testEncoder(field, dataWords, ecWords);
  _testDecoder(field, dataWords, ecWords);
}
...
void _testEncoder(GenericGF field, List<int> dataWords, List<int> ecWords) {
  ...
  system.arraycopy(
      ecWords, 0, messageExpected, dataWords.length, ecWords.length);
  system.arraycopy(dataWords, 0, message, 0, dataWords.length);
  encoder.encode(message, ecWords.length);
  _expectDataEquals(message, messageExpected,
      reason:
