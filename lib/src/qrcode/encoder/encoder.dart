...
class Encoder {
  ...
  static final CharacterSetECI DEFAULT_BYTE_MODE_ENCODING = CharacterSetECI.ISO8859_1;
   ...
   */
   static QRCode encode(String content,
    ...
    bool hasEncodingHint = hints != null && hints.contains(EncodeHintType.CHARACTER_SET);
    if (hasEncodingHint) {
      encoding = CharacterSetECI.findByName(hints.get(EncodeHintType.CHARACTER_SET)!);
    }

    // Pick an encoding mode appropriate for the content. Note that this will not attempt to use
    // multiple modes / segments even if that were more efficient. Twould be nice.
    Mode mode = chooseMode(content, encoding.encoding);
    ...
    if (hints.get(EncodeHintType.GS1_FORMAT) ?? false) {
    ...
    Version version;
    int? versionNumber = hints.get(EncodeHintType.QR_VERSION);
    ...
    int numDataBytes = version.totalCodewords - ecBlocks.getTotalECCodewords();
